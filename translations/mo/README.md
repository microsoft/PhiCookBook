...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-17T10:09:34+00:00",
  "source_file": "README.md",
  "language_code": "mo"
}
-->
# Phi 食譜：使用 Microsoft 的 Phi 模型的實作範例
...
[![GitHub 星星](https://img.shields.io/github/stars/microsoft/phicookbook?style=social&label=Star)](https://GitHub.com/microsoft/phicookbook/stargazers/?WT.mc_id=aiml-137032-kinfeylo)
...
按照以下步驟開始使用這些資源：
1. **分叉此存儲庫**：點擊 [![GitHub 分叉](https://img.shields.io/github/forks/microsoft/phicookbook.svg?style=social&label=Fork)](https://GitHub.com/microsoft/phicookbook/network/?WT.mc_id=aiml-137032-kinfeylo)
...
- 介紹
  ...
  - [Phi 硬體支持](./md/01.Introduction/01/01.Hardwaresupport.md)
...
- Phi 家族推理
    ...
    - [使用 Kaito AKS、Azure Containers（官方支持）推理 Phi](./md/01.Introduction/03/Kaito_Inference.md)
-  [量化 Phi 家族](./md/01.Introduction/04/QuantifyingPhi.md)
    - [使用 llama.cpp 量化 Phi-3.5 / 4](./md/01.Introduction/04/UsingLlamacppQuantifyingPhi.md)
    - [使用 Generative AI 擴展 for onnxruntime 量化 Phi-3.5 / 4](./md/01.Introduction/04/UsingORTGenAIQuantifyingPhi.md)
...
- [在 Azure AI Foundry 中評估微調後的 Phi-3 / Phi-3.5 模型，聚焦於 Microsoft 負責任 AI 原則](./md/02.Application/01.TextAndChat/Phi3/E2E_Phi-3-Evaluation_AIFoundry.md)
...
- Azure AI 推理 SDK 程式碼範例
  ...
  - Phi-3 / 3.5 範例
    - [建立自己的 Visual Studio Code GitHub Copilot Chat，使用 Microsoft Phi-3 系列](./md/02.Application/02.Code/Phi3/VSCodeExt/README.md)
    - [使用 Phi-3.5 和 GitHub 模型建立自己的 Visual Studio Code Chat Copilot Agent](/md/02.Application/02.Code/Phi3/CreateVSCodeChatAgentWithGitHubModels.md)
...
- 視覺範例
  ...
  - Phi-3 / 3.5 範例
    ...
    - [基於選單的使用 Microsoft.ML.OnnxRuntime .NET 的本地 ONNX Phi-3 Vision 模型](../../md/04.HOL/dotnet/src/LabsPhi304)
...
- Phi 微調範例
  ...
  - [使用 AI 工具包微調 Phi-3，適用於 VS Code](./md/03.FineTuning/Finetuning_VSCodeaitoolkit.md)
...
- 學術研究論文與出版物
  ...
  - [(WhyPHI) 微調 PHI-3 用於多選題回答：方法論、結果及挑戰](https://arxiv.org/abs/2501.01588)
...
您可以學習如何使用 Microsoft Phi 並在不同的硬件設備上構建端到端解決方案。要親自體驗 Phi，請先試用模型並根據您的場景自定義 Phi，使用 [Azure AI Foundry Azure AI Model Catalog](https://aka.ms/phi3-azure-ai)。您可以在[Azure AI Foundry 快速入門指南](/md/02.QuickStart/AzureAIFoundry_QuickStart.md)中了解更多。
...
您可以學習如何使用 Microsoft Phi 並在不同的硬件設備上構建端到端解決方案。要親自體驗 Phi，請先試用模型並根據您的場景自定義 Phi，使用 [GitHub Model Catalog](https://github.com/marketplace/models?WT.mc_id=aiml-137032-kinfeylo)。您可以在[GitHub Model Catalog 快速入門指南](/md/02.QuickStart/GitHubModel_QuickStart.md)中了解更多。
...
Microsoft 致力於幫助客戶負責任地使用我們的 AI 產品，分享我們的經驗，並通過透明度說明和影響評估等工具建立基於信任的合作關係。許多這些資源可以在 [https://aka.ms/RAI](https://aka.ms/RAI) 找到。  
Microsoft 的負責任 AI 方法基於我們的 AI 原則，包括公平性、可靠性和安全性、隱私和安全性、包容性、透明度和問責性。

大規模自然語言、圖像和語音模型（如本示例中使用的模型）可能會以不公平、不可靠或冒犯的方式行為，從而造成傷害。請參閱 [Azure OpenAI 服務透明度說明](https://learn.microsoft.com/legal/cognitive-services/openai/transparency-note?tabs=text)，了解風險和限制。

減輕這些風險的建議方法是在您的架構中包含一個安全系統，該系統可以檢測並防止有害行為。[Azure AI Content Safety](https://learn.microsoft.com/azure/ai-services/content-safety/overview) 提供了一個獨立的保護層，能夠檢測應用程序和服務中的用戶生成和 AI 生成的有害內容。Azure AI Content Safety 包括文本和圖像 API，允許您檢測有害材料。在 Azure AI Foundry 中，Content Safety 服務允許您查看、探索和試用檢測不同模態有害內容的示例代碼。以下[快速入門文檔](https://learn.microsoft.com/azure/ai-services/content-safety/quickstart-text?tabs=visual-studio%2Clinux&pivots=programming-language-rest)將指導您向服務發送請求。

需要考慮的另一個方面是整體應用程序性能。對於多模態和多模型應用程序，我們認為性能意味著系統按您和您的用戶期望的方式運行，包括不生成有害輸出。重要的是使用[性能和質量以及風險和安全評估工具](https://learn.microsoft.com/azure/ai-studio/concepts/evaluation-metrics-built-in)評估整體應用程序的性能。您還可以使用[自定義評估工具](https://learn.microsoft.com/azure/ai-studio/how-to/develop/evaluate-sdk#custom-evaluators)進行創建和評估。

您可以在開發環境中使用 [Azure AI Evaluation SDK](https://microsoft.github.io/promptflow/index.html) 評估您的 AI 應用程序。給定測試數據集或目標，您的生成式 AI 應用程序生成的內容可以通過內置評估工具或您選擇的自定義評估工具進行定量測量。要開始使用 Azure AI Evaluation SDK 評估您的系統，您可以遵循[快速入門指南](https://learn.microsoft.com/azure/ai-studio/how-to/develop/flow-evaluate-sdk)。執行評估運行後，您可以[在 Azure AI Foundry 中可視化結果](https://learn.microsoft.com/azure/ai-studio/how-to/evaluate-flow-results)。
