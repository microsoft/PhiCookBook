...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T06:19:18+00:00",
...
[Kaito](https://github.com/Azure/kaito) 是一個在 Kubernetes 叢集中自動化 AI/ML 推論模型部署的運營工具。

與大多數基於虛擬機基礎架構的主流模型部署方法相比，Kaito 具有以下主要差異：

- 使用容器映像管理模型檔案。提供一個 HTTP 伺服器，透過模型庫執行推論請求。
...
Kaito 採用經典的 Kubernetes 自訂資源定義（CRD）/控制器設計模式。使用者管理一個描述 GPU 需求及推論規格的 `workspace` 自訂資源。Kaito 控制器會透過調和 `workspace` 自訂資源來自動化部署。
<div align="left">
...
- **Workspace controller**：調和 `workspace` 自訂資源，建立 `machine`（如下所述）自訂資源以觸發節點自動配置，並根據模型預設配置建立推論工作負載（`deployment` 或 `statefulset`）。
...
> 注意：[ *gpu-provisioner*](https://github.com/Azure/gpu-provisioner) 是一個開源元件。若其他控制器支援 [Karpenter-core](https://sigs.k8s.io/karpenter) API，也可以替代它。
...
可透過執行以下指令追蹤 workspace 狀態。當 WORKSPACEREADY 欄位顯示為 `True`，表示模型已成功部署。
