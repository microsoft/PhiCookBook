...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-07T10:17:10+00:00",
...
> [!NOTE]  
> Dieses Labor dauert **35 Minuten** und bietet Ihnen eine praktische Einführung in die Kernkonzepte der Modelloptimierung für die Inferenz auf dem Gerät mit OLIVE.

## Lernziele

Am Ende dieses Labors können Sie OLIVE verwenden, um:

- Ein KI-Modell mit der AWQ-Quantisierungsmethode zu quantisieren.  
- Ein KI-Modell für eine spezifische Aufgabe feinabzustimmen.  
- LoRA-Adapter (feinabgestimmtes Modell) für eine effiziente Inferenz auf dem Gerät mit dem ONNX Runtime zu erzeugen.

### Was ist Olive

Olive (*O*NNX *live*) ist ein Toolkit zur Modelloptimierung mit begleitender CLI, mit dem Sie Modelle für die ONNX Runtime +++https://onnxruntime.ai+++ mit Qualität und Leistung bereitstellen können.

![Olive Flow](../../../../../translated_images/olive-flow.a47985655a756dcba73521511ea42eef359509a3a33cbd4b9ac04ba433287b80.de.png)

Die Eingabe für Olive ist typischerweise ein PyTorch- oder Hugging Face-Modell, und die Ausgabe ist ein optimiertes ONNX-Modell, das auf einem Gerät (Zielplattform) mit ONNX Runtime ausgeführt wird. Olive optimiert das Modell für den AI-Beschleuniger (NPU, GPU, CPU) der Zielplattform, bereitgestellt von einem Hardwareanbieter wie Qualcomm, AMD, Nvidia oder Intel.

Olive führt einen *Workflow* aus, eine geordnete Abfolge einzelner Modelloptimierungsaufgaben, sogenannte *passes* – Beispiele für solche passes sind: Modellkompression, Graph-Erfassung, Quantisierung, Graph-Optimierung. Jeder pass hat eine Reihe von Parametern, die angepasst werden können, um die besten Kennzahlen wie Genauigkeit und Latenz zu erreichen, welche vom jeweiligen Evaluator bewertet werden. Olive verwendet eine Suchstrategie, die einen Suchalgorithmus einsetzt, um jeden pass einzeln oder mehrere passes zusammen automatisch zu optimieren.

#### Vorteile von Olive

- **Reduziert Frustration und Zeit** bei manuellen Trial-and-Error-Experimenten mit verschiedenen Techniken zur Graph-Optimierung, Kompression und Quantisierung. Definieren Sie Ihre Qualitäts- und Leistungsanforderungen und lassen Sie Olive automatisch das beste Modell für Sie finden.  
...
- Modell-Paketierung und Bereitstellung sind integriert.  
- Unterstützt die Erzeugung von Modellen für **Multi LoRA Serving**.  
- Workflows lassen sich mit YAML/JSON erstellen, um Modelloptimierung und Bereitstellungsaufgaben zu orchestrieren.  
...
> [!NOTE]  
> Bitte stellen Sie sicher, dass Sie Ihr Azure AI Hub und Projekt eingerichtet haben und Ihren A100-Compute gemäß Labor 1 konfiguriert haben.
...
1. Folgen Sie den Anweisungen auf dem Bildschirm, um die Verbindung zum Compute herzustellen. Dabei wählen Sie Ihr Azure-Abonnement, Ihre Ressourcengruppe, Ihr Projekt und den Compute-Namen, den Sie in Labor 1 eingerichtet haben.  
...
Um VS Code im entsprechenden Ordner zu öffnen, führen Sie im Terminal den folgenden Befehl aus, der ein neues Fenster öffnet:
...
In diesem Labor laden Sie Modelle in den Azure AI Model-Katalog hoch und laden sie herunter. Um auf den Modellkatalog zugreifen zu können, müssen Sie sich bei Azure anmelden mit:
...
> [!NOTE]  
> Bei der Anmeldung werden Sie aufgefordert, Ihr Abonnement auszuwählen. Stellen Sie sicher, dass Sie das für dieses Labor bereitgestellte Abonnement auswählen.
...
1. **Daten inspizieren:** In diesem Beispiel werden Sie das Phi-3.5-Mini-Modell feinabstimmen, damit es auf Fragen zum Thema Reisen spezialisiert ist. Der folgende Code zeigt die ersten Datensätze des Datensatzes, der im JSON-Lines-Format vorliegt:

    ```bash
    head data/data_sample_travel.jsonl
    ```

1. **Modell quantisieren:** Bevor Sie das Modell trainieren, quantisieren Sie es mit folgendem Befehl, der eine Technik namens Active Aware Quantization (AWQ) +++https://arxiv.org/abs/2306.00978+++ verwendet. AWQ quantisiert die Gewichte eines Modells unter Berücksichtigung der Aktivierungen, die während der Inferenz erzeugt werden. Das bedeutet, dass der Quantisierungsprozess die tatsächliche Datenverteilung in den Aktivierungen berücksichtigt, was zu einer besseren Erhaltung der Modellgenauigkeit im Vergleich zu herkömmlichen Gewicht-Quantisierungsmethoden führt.
    ...
    In diesem Labor zeigen wir Ihnen, wie Sie Modelle von Hugging Face eingeben (zum Beispiel: `microsoft/Phi-3.5-mini-instruct`). However, Olive also allows you to input models from the Azure AI catalog by updating the `model_name_or_path` argument to an Azure AI asset ID (for example:  `azureml://registries/azureml/models/Phi-3.5-mini-instruct/versions/4`). 

1. **Train the model:** Next, the `olive finetune`-Befehl feinjustiert das quantisierte Modell. Die Quantisierung vor der Feinabstimmung führt zu besserer Genauigkeit, da der Feinabstimmungsprozess einige Verluste durch die Quantisierung ausgleicht.
...
Um die Inferenz des Modells zu testen, erstellen Sie in Ihrem Ordner eine Python-Datei namens **app.py** und fügen Sie folgenden Code ein:
...
Das Hochladen des Modells in ein Azure AI Modell-Repository macht das Modell für andere Mitglieder Ihres Entwicklungsteams zugänglich und verwaltet auch die Versionskontrolle des Modells. Um das Modell hochzuladen, führen Sie folgenden Befehl aus:

> [!NOTE]  
> Aktualisieren Sie die Platzhalter `{}` für `resourceGroup` und den Namen des Azure AI Projekts und führen Sie dann den Befehl aus.

```
az ml workspace show
```

Alternativ können Sie auch auf +++ai.azure.com+++ gehen und **Management Center** > **Projekt** > **Übersicht** auswählen.

Ersetzen Sie die `{}`-Platzhalter durch den Namen Ihrer Ressourcengruppe und des Azure AI Projekts.
