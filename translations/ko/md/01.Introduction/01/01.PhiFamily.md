...
Phi 모델은 가장 유능하고 비용 효율적인 소형 언어 모델(SLM)로, 다양한 언어, 추론, 코딩, 오디오, 비전, 수학 벤치마크에서 동일 크기 및 그보다 한 단계 큰 크기 모델보다 우수한 성능을 보여줍니다. 이번 출시로 고객에게 고품질 모델 선택의 폭을 넓혀, 생성형 AI 애플리케이션을 구성하고 구축하는 데 있어 보다 실용적인 선택지를 제공합니다.

Phi 패밀리는 파이썬 코드 생성용 Phi-1으로 시작하여 텍스트 및 채팅 완료 기반의 Phi-1.5 /2, Phi-3-mini/small/medium-instruct와 Phi-3.5/4-mini-instruct까지 이어졌으며, 비전을 위한 Phi-3/3.5-vision, 강력한 추론 기반의 Phi-4, MoE용 Phi-3.5-MoE, 그리고 현재는 전 모달 모델 Phi-4-multimodal로 발전했습니다. 고품질 데이터 세트를 통해 벤치마크 학습이 더 큰 학습 파라미터를 지닌 모델과 비견될 수 있습니다.
...
| Model Card |Parameters|코딩|텍스트/채팅 완료|고급 추론| 비전 | 오디오 | MoE
| - | -  | - | - |- |- |- |- |
...
|[Phi-4-reasoning-vision](https://huggingface.co/microsoft/Phi-4-reasoning-vision-15B)|15B|아니오|아니오| 예 | 예 |아니오 |아니오 |
...
- [마이크로소프트 파운드리 모델 카탈로그](https://ai.azure.com/explore/models?selectedCollection=phi)
- [GitHub 모델](https://github.com/marketplace?query=Phi&type=models)
- 허깅페이스
...
| | | | |
|-|-|-|-|
|고객 요구|작업|시작 모델|자세한 내용|
|단순히 메시지 스레드를 요약하는 모델 필요|대화 요약|Phi-3 / 3.5 텍스트 모델|여기서 결정 요인은 고객이 명확하고 직관적인 언어 작업을 가지고 있다는 점입니다|
|어린이용 무료 수학 튜터 앱|수학 및 추론|Phi-3 / 3.5 / 4 텍스트 모델|무료 앱이라서 고객이 반복 과금 없이 해결책을 원하기 때문입니다 |
|자가 순찰 자동차 카메라|비전 분석|Phi-3 /3.5 -Vision 또는 Phi-4-multimodal|인터넷 없이 엣지에서 작동 가능한 솔루션이 필요합니다|
|AI 기반 여행 예약 에이전트를 구축하고자 함|복잡한 계획, 함수 호출 및 조정 필요|GPT 모델|정보 수집을 위한 계획, API 호출 및 실행 기능이 필요합니다 |
|직원용 코파일럿 구축 원함|RAG, 다중 도메인, 복잡하고 개방형|GPT 모델 + Phi 패밀리|개방형 시나리오, 광범위한 세계 지식 필요, 따라서 더 큰 모델이 적합합니다. 지식 내용 분할이 필요하며, 아마도 SLM이 적합할 수 있습니다 |

---

<!-- CO-OP TRANSLATOR DISCLAIMER START -->
**면책 조항**:  
이 문서는 AI 번역 서비스 [Co-op Translator](https://github.com/Azure/co-op-translator)를 사용하여 번역되었습니다. 정확성을 위해 노력하고 있으나, 자동 번역에는 오류나 부정확한 내용이 포함될 수 있음을 유의하시기 바랍니다. 원문 문서는 해당 언어의 권위 있는 자료로 간주되어야 합니다. 중요한 정보의 경우 전문가의 번역을 권장합니다. 이 번역 사용으로 인해 발생하는 오해나 잘못된 해석에 대해 저희는 책임을 지지 않습니다.
