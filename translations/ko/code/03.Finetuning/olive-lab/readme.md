...
Olive(*O*NNX *live*)는 ONNX 런타임 +++https://onnxruntime.ai+++용 모델을 품질과 성능을 갖추어 배포할 수 있도록 도와주는 모델 최적화 툴킷과 CLI입니다.

![Olive Flow](../../../../../translated_images/ko/olive-flow.a47985655a756dcb.webp)
...
Olive는 *워크플로우*를 실행하는데, 이는 *패스*라 불리는 개별 모델 최적화 작업들의 순서 있는 연속입니다. 예를 들어 모델 압축, 그래프 캡처, 양자화, 그래프 최적화 등이 있습니다. 각 패스는 정확도와 지연 시간 같은 지표를 평가하는 평가자가 사용하는 최적의 매개변수를 조정할 수 있습니다. Olive는 탐색 알고리즘을 사용해 각 패스를 하나씩 또는 여러 패스를 함께 자동 조정하는 탐색 전략을 사용합니다.

#### Olive의 장점

- 그래프 최적화, 압축, 양자화 등 다양한 기법을 수동으로 시행착오하며 실험하는 데 드는 **시간과 스트레스 감소**. 품질과 성능 제약 조건을 정의하면 Olive가 자동으로 최적의 모델을 찾아줍니다.
...
5. 연결되면 Visual Code 왼쪽 하단에 `><Azure ML: Compute Name`이 표시됩니다.

### 1단계: 이 저장소 클론하기

VS Code에서 **Ctrl+J**로 새 터미널을 열고 이 저장소를 클론하세요:
...
터미널에서 다음 명령어를 실행하면 새 창에서 해당 폴더가 열립니다:
...
1. **데이터 확인하기:** 이 예제에서는 Phi-3.5-Mini 모델을 여행 관련 질문에 특화되도록 미세 조정할 예정입니다. 아래 코드는 JSON 라인 형식의 데이터셋 첫 몇 개 레코드를 보여줍니다:

    ```bash
    head data/data_sample_travel.jsonl
    ```

2. **모델 양자화하기:** 모델을 학습하기 전에 Active Aware Quantization (AWQ) +++https://arxiv.org/abs/2306.00978+++ 기법을 사용해 다음 명령어로 양자화합니다. AWQ는 추론 중 생성되는 활성화를 고려해 모델 가중치를 양자화합니다. 즉, 활성화의 실제 데이터 분포를 반영해 전통적인 가중치 양자화보다 모델 정확도를 더 잘 유지합니다.
...
3. **모델 학습하기:** 다음으로 `olive finetune` 명령어로 양자화된 모델을 미세 조정합니다. 양자화 후가 아니라 *양자화 전에* 미세 조정하는 것이 더 좋은 정확도를 내는데, 미세 조정 과정에서 양자화로 인한 손실 일부를 회복하기 때문입니다.
...
4. **최적화하기:** 모델 학습이 끝나면 Olive의 `auto-opt` 명령어로 모델을 최적화합니다. 이 명령어는 ONNX 그래프를 캡처하고 모델을 압축 및 융합해 CPU 성능을 향상시키는 여러 최적화를 자동으로 수행합니다. 참고로 `--device`와 `--provider` 인자를 바꾸면 NPU나 GPU 등 다른 디바이스용 최적화도 가능합니다. 하지만 이 랩에서는 CPU를 사용합니다.
...
리소스 그룹 `"resourceGroup"`과 Azure AI 프로젝트 이름을 확인하려면 다음 명령어를 실행하거나
