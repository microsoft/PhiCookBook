# Microsoft Foundry में Microsoft के Responsible AI सिद्धांतों पर केंद्रित Fine-tuned Phi-3 / Phi-3.5 मॉडल का मूल्यांकन
...
### Microsoft Foundry में Fine-tuned Phi-3 / Phi-3.5 मॉडल की सुरक्षा और प्रदर्शन का मूल्यांकन कैसे करें?

किसी मॉडल को फाइन-ट्यून करने पर कभी-कभी अनचाहे या अवांछित प्रतिक्रियाएं आ सकती हैं। यह सुनिश्चित करने के लिए कि मॉडल सुरक्षित और प्रभावी बना रहे, यह जरूरी है कि मॉडल की हानिकारक सामग्री उत्पन्न करने की संभावना और सटीक, प्रासंगिक, और संगत प्रतिक्रियाएं देने की क्षमता का मूल्यांकन किया जाए। इस ट्यूटोरियल में, आप सीखेंगे कि Microsoft Foundry में Prompt flow के साथ एकीकृत Fine-tuned Phi-3 / Phi-3.5 मॉडल की सुरक्षा और प्रदर्शन का मूल्यांकन कैसे करें।

यहाँ Microsoft Foundry का मूल्यांकन प्रक्रिया है।
...
1. [**परिदृश्य 1: Microsoft Foundry के Prompt flow मूल्यांकन का परिचय**](../../../../../../md/02.Application/01.TextAndChat/Phi3)
...
1. [**परिदृश्य 2: Microsoft Foundry में Phi-3 / Phi-3.5 मॉडल का मूल्यांकन**](../../../../../../md/02.Application/01.TextAndChat/Phi3)
    ...
    - [Microsoft Foundry के Prompt flow मूल्यांकन का उपयोग करके Fine-tuned Phi-3 / Phi-3.5 मॉडल का मूल्यांकन करें](../../../../../../md/02.Application/01.TextAndChat/Phi3)
...
यह सुनिश्चित करने के लिए कि आपका AI मॉडल नैतिक और सुरक्षित है, इसे Microsoft के Responsible AI सिद्धांतों के अनुसार मूल्यांकन करना आवश्यक है। Microsoft Foundry में, सुरक्षा मूल्यांकन आपको आपके मॉडल की jailbreak हमलों के प्रति संवेदनशीलता और हानिकारक सामग्री उत्पन्न करने की क्षमता का आकलन करने की अनुमति देते हैं, जो सीधे इन सिद्धांतों के अनुरूप है।
...
इस ट्यूटोरियल में, आप Microsoft Foundry के सुरक्षा मेट्रिक्स का उपयोग करके Fine-tuned Phi-3 मॉडल की सुरक्षा का मूल्यांकन करेंगे। ये मेट्रिक्स मॉडल की हानिकारक सामग्री उत्पन्न करने की क्षमता और jailbreak हमलों के प्रति उसकी संवेदनशीलता का आकलन करने में मदद करते हैं। सुरक्षा मेट्रिक्स में शामिल हैं:
...
यह सुनिश्चित करने के लिए कि आपका AI मॉडल अपेक्षित प्रदर्शन कर रहा है, इसके प्रदर्शन का प्रदर्शन मेट्रिक्स के आधार पर मूल्यांकन करना महत्वपूर्ण है। Microsoft Foundry में, प्रदर्शन मूल्यांकन आपको आपके मॉडल की सटीक, प्रासंगिक, और संगत प्रतिक्रियाएं उत्पन्न करने की क्षमता का आकलन करने की अनुमति देते हैं।
...
इस ट्यूटोरियल में, आप Microsoft Foundry के प्रदर्शन मेट्रिक्स का उपयोग करके Fine-tuned Phi-3 / Phi-3.5 मॉडल के प्रदर्शन का मूल्यांकन करेंगे। ये मेट्रिक्स मॉडल की सटीक, प्रासंगिक, और संगत प्रतिक्रियाएं उत्पन्न करने की क्षमता का आकलन करने में मदद करते हैं। प्रदर्शन मेट्रिक्स में शामिल हैं:
...
यह ट्यूटोरियल पिछले ब्लॉग पोस्ट "[Fine-Tune and Integrate Custom Phi-3 Models with Prompt Flow: Step-by-Step Guide](https://techcommunity.microsoft.com/t5/educator-developer-blog/fine-tune-and-integrate-custom-phi-3-models-with-prompt-flow/ba-p/4178612?wt.mc_id=studentamb_279723)" और "[Fine-Tune and Integrate Custom Phi-3 Models with Prompt Flow in Microsoft Foundry](https://techcommunity.microsoft.com/t5/educator-developer-blog/fine-tune-and-integrate-custom-phi-3-models-with-prompt-flow-in/ba-p/4191726?wt.mc_id=studentamb_279723)" का अनुसरण है। इन पोस्ट्स में, हमने Microsoft Foundry में Phi-3 / Phi-3.5 मॉडल को फाइन-ट्यून करने और इसे Prompt flow के साथ एकीकृत करने की प्रक्रिया को समझाया था।

इस ट्यूटोरियल में, आप Microsoft Foundry में एक मूल्यांकनकर्ता के रूप में Azure OpenAI मॉडल को तैनात करेंगे और इसका उपयोग अपने Fine-tuned Phi-3 / Phi-3.5 मॉडल का मूल्यांकन करने के लिए करेंगे।
...
#### Microsoft Foundry में Prompt flow के साथ कस्टम Phi-3 / Phi-3.5 मॉडल को एकीकृत करें (Code first approach)
> [!NOTE]  
...
आगे बढ़ने के लिए, आपको अपने फाइन-ट्यून किए गए Phi-3 / Phi-3.5 मॉडल को Microsoft Foundry में Prompt flow में इंटीग्रेट करना होगा।

#### Microsoft Foundry Hub बनाएं

प्रोजेक्ट बनाने से पहले आपको एक Hub बनाना होगा। Hub एक Resource Group की तरह काम करता है, जो आपको Microsoft Foundry के अंदर कई प्रोजेक्ट्स को व्यवस्थित और प्रबंधित करने की सुविधा देता है।
...
1. [Microsoft Foundry](https://ai.azure.com/?wt.mc_id=studentamb_279723) पर जाएं।

1. उस Microsoft Foundry प्रोजेक्ट पर नेविगेट करें जिसे आपने बनाया है।
...
आपने Microsoft Foundry में एक कस्टम कनेक्शन जोड़ दिया है। अब, निम्नलिखित चरणों का उपयोग करके एक Prompt flow बनाएं। फिर, आप इस Prompt flow को कस्टम कनेक्शन से कनेक्ट करेंगे ताकि फाइन-ट्यून किए गए मॉडल का उपयोग Prompt flow के अंदर किया जा सके।
...
> [!NOTE]
> Microsoft Foundry में Prompt flow के उपयोग के बारे में अधिक विस्तृत जानकारी के लिए, आप [Prompt flow in Microsoft Foundry](https://learn.microsoft.com/azure/ai-studio/how-to/prompt-flow) देख सकते हैं।
...
Phi-3 / Phi-3.5 मॉडल का Microsoft Foundry में मूल्यांकन करने के लिए, आपको एक Azure OpenAI मॉडल डिप्लॉय करना होगा। इस मॉडल का उपयोग Phi-3 / Phi-3.5 मॉडल के प्रदर्शन का मूल्यांकन करने के लिए किया जाएगा।
...
### Microsoft Foundry के Prompt flow मूल्यांकन का उपयोग करके फाइन-ट्यून किए गए Phi-3 / Phi-3.5 मॉडल का मूल्यांकन करें
