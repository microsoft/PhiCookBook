...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T15:45:37+00:00",
...
![Olive Flow](../../../../../translated_images/es/olive-flow.a47985655a756dcb.webp)

La entrada a Olive suele ser un modelo de PyTorch o Hugging Face y la salida es un modelo ONNX optimizado que se ejecuta en un dispositivo (objetivo de despliegue) con ONNX runtime. Olive optimiza el modelo para el acelerador de IA del dispositivo (NPU, GPU, CPU) proporcionado por un fabricante de hardware como Qualcomm, AMD, Nvidia o Intel.
...
5. Una vez conectado a tu nodo de Azure ML Compute, se mostrará en la **parte inferior izquierda de Visual Code** `><Azure ML: Nombre del Compute`
...
2. **Cuantiza el modelo:** Antes de entrenar el modelo, primero lo cuantizas con el siguiente comando que usa una técnica llamada Active Aware Quantization (AWQ) +++https://arxiv.org/abs/2306.00978+++. AWQ cuantiza los pesos de un modelo considerando las activaciones producidas durante la inferencia. Esto significa que el proceso de cuantización toma en cuenta la distribución real de los datos en las activaciones, lo que lleva a una mejor preservación de la precisión del modelo comparado con métodos tradicionales de cuantización de pesos.
...
3. **Entrena el modelo:** A continuación, el comando `olive finetune` afina el modelo cuantizado. Cuantizar el modelo *antes* de afinarlo en lugar de después da mejor precisión, ya que el proceso de afinamiento recupera parte de la pérdida causada por la cuantización.
...
4. **Optimiza:** Con el modelo entrenado, ahora optimízalo usando el comando `auto-opt` de Olive, que capturará el grafo ONNX y realizará automáticamente varias optimizaciones para mejorar el rendimiento del modelo en CPU comprimiendo el modelo y haciendo fusiones. Cabe destacar que también puedes optimizar para otros dispositivos como NPU o GPU simplemente actualizando los argumentos `--device` y `--provider` — pero para este laboratorio usaremos CPU.
...
O accediendo a +++ai.azure.com+++ y seleccionando **centro de administración** > **proyecto** > **visión general**
