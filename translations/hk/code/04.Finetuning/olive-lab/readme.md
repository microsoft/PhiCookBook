...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T16:10:13+00:00",
...
Olive（*O*NNX *live*）是一套模型優化工具包，搭配 CLI，讓你能夠為 ONNX runtime +++https://onnxruntime.ai+++ 打包高品質且高效能的模型。

![Olive 流程圖](../../../../../translated_images/olive-flow.c4f76d9142c579b2462b631b8aa862093b595bb89064fa33e6d4fa90f937f52d.hk.png)
...
Olive 執行一個 *workflow*，即一連串有序的模型優化任務，稱為 *passes*。範例 passes 包括：模型壓縮、圖形捕捉、量化、圖形優化。每個 pass 都有一組可調整的參數，用以達成最佳指標，例如準確度和延遲，這些指標由相應的評估器評估。Olive 採用搜尋策略，利用搜尋演算法逐一或同時自動調整各個 pass。

#### Olive 的優點

- **減少手動嘗試錯誤的挫折與時間**，無需自行實驗各種圖形優化、壓縮和量化技術。只需定義品質與效能限制，Olive 就會自動幫你找到最佳模型。
...
4. 按照畫面指示連接計算資源，過程中需選擇你的 Azure 訂閱、資源群組、專案及你在實驗室 1 中設定的計算資源名稱。  
5. 連接成功後，Visual Code 左下角會顯示 `><Azure ML: Compute Name`。

### 步驟 1：複製此儲存庫

在 VS Code 中，使用 **Ctrl+J** 開啟新的終端機，並複製此儲存庫：
...
在終端機執行以下指令，會在新視窗中開啟相關資料夾：

```bash
code phi-3cookbook/code/04.Finetuning/Olive-lab
```

或者，你也可以透過選單 **檔案** > **開啟資料夾** 來開啟。
...
2. **量化模型：** 在訓練模型前，先使用 Active Aware Quantization (AWQ) 技術 +++https://arxiv.org/abs/2306.00978+++ 進行量化。AWQ 會考慮推理時產生的激活值，根據激活的實際資料分布來量化權重，較傳統權重量化方法能更好地保留模型準確度。
...
4. **優化模型：** 模型訓練完成後，使用 Olive 的 `auto-opt` 指令優化模型，該指令會捕捉 ONNX 圖形並自動執行多項優化，透過壓縮與融合提升 CPU 上的模型效能。你也可以透過更新 `--device` 和 `--provider` 參數來優化其他裝置（如 NPU 或 GPU），但本實驗室以 CPU 為例。
...
為測試模型推理，在資料夾中建立名為 **app.py** 的 Python 檔案，並貼上以下程式碼：
