...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T05:57:40+00:00",
...
讓我們來看看如何使用 Azure AI Foundry 微調 Microsoft 的 Phi-3 Mini 語言模型。微調能讓你將 Phi-3 Mini 調整至特定任務，使其更強大且更具上下文感知能力。

## 注意事項

- **功能：** 哪些模型可以微調？基礎模型能被微調成什麼樣的功能？
- **成本：** 微調的收費模式是什麼？
- **可定制性：** 我能修改基礎模型多少？以什麼方式修改？
- **便利性：** 微調的流程是怎樣的？我需要寫自訂程式碼嗎？需要自備運算資源嗎？
- **安全性：** 微調後的模型存在安全風險嗎？有沒有防護機制避免意外傷害？

![AIFoundry Models](../../../../translated_images/hk/AIFoundryModels.0e1b16f7d0b09b73.webp)
...
- Azure 角色型存取控制 (Azure RBAC) 用於授權 Azure AI Foundry 的操作。要執行本文的步驟，你的使用者帳號必須在資源群組中被指派為 __Azure AI Developer 角色__。
...
5. 確認列表中有 **Microsoft.Network**，若沒有則新增。
...
![Finetune AI Foundry](../../../../translated_images/hk/AIFoundryfinetune.193aaddce48d553c.webp)
...
2. 選擇 **+New project** 以在 Azure AI Foundry 建立新專案。
    ...
    ![FineTuneSelect](../../../../translated_images/hk/select-new-project.cd31c0404088d7a3.webp)
...
3. 執行以下操作：
    ...
    ![FineTuneSelect](../../../../translated_images/hk/create-project.ca3b71298b90e420.webp)
...
4. 建立新 hub 時，請執行以下操作：
    ...
    ![FineTuneSelect](../../../../translated_images/hk/create-hub.49e53d235e80779e.webp)
...
微調前，收集或建立與任務相關的資料集，例如聊天指令、問答對或其他相關文字資料。清理並預處理資料，移除雜訊、處理缺失值，並進行文字分詞。
...
2. 在 **搜尋欄** 輸入 *phi-3*，並選擇你想使用的 phi-3 模型。
    ...
    ![FineTuneSelect](../../../../translated_images/hk/select-model.60ef2d4a6a3cec57.webp)

3. 選擇 **Fine-tune**。

    ![FineTuneSelect](../../../../translated_images/hk/select-finetune.a976213b543dd9d8.webp)

4. 輸入 **Fine-tuned model name**。

    ![FineTuneSelect](../../../../translated_images/hk/finetune1.c2b39463f0d34148.webp)
...
6. 執行以下操作：
    ...
    ![FineTuneSelect](../../../../translated_images/hk/finetune2.43cb099b1a94442d.webp)

7. 選擇 **Next**。

8. 上傳你想使用的 **Validation data**，或選擇 **Automatic split of training data**。

    ![FineTuneSelect](../../../../translated_images/hk/finetune3.fd96121b67dcdd92.webp)
...
10. 執行以下操作：
    ...
    ![FineTuneSelect](../../../../translated_images/hk/finetune4.e18b80ffccb5834a.webp)

11. 選擇 **Submit** 開始微調流程。

    ![FineTuneSelect](../../../../translated_images/hk/select-submit.0a3802d581bac271.webp)
...
12. 微調完成後，狀態會顯示為 **Completed**，如下圖所示。你現在可以部署模型，並在自己的應用程式、遊樂場或提示流程中使用。更多資訊請參考 [如何使用 Azure AI Foundry 部署 Phi-3 系列小型語言模型](https://learn.microsoft.com/azure/ai-studio/how-to/deploy-models-phi-3?tabs=phi-3-5&pivots=programming-language-python)。
    ...
    ![FineTuneSelect](../../../../translated_images/hk/completed.4dc8d2357144cdef.webp)
...
你可以從 [Azure AI Foundry](https://ai.azure.com) 的微調模型清單或模型詳細頁面刪除微調模型。從微調頁面選擇要刪除的微調模型，然後按下刪除按鈕即可。
...
Phi 模型作為服務微調由 Microsoft 提供，並整合於 Azure AI Foundry 使用。你可以在[部署](https://learn.microsoft.com/azure/ai-studio/how-to/deploy-models-phi-3?tabs=phi-3-5&pivots=programming-language-python)或微調模型時，在部署精靈的「價格與條款」標籤中查看收費資訊。

## 內容過濾

以按用量付費方式部署的服務模型受 Azure AI Content Safety 保護。部署至即時端點時，你可以選擇關閉此功能。啟用 Azure AI Content Safety 後，提示詞與完成結果都會通過一組分類模型，目的是偵測並防止輸出有害內容。內容過濾系統會偵測並對輸入提示與輸出完成中的特定潛在有害內容類別採取行動。了解更多 [Azure AI Content Safety](https://learn.microsoft.com/azure/ai-studio/concepts/content-filtering)。
...
自訂任務：Phi-3 Mini 可微調用於聊天指令以外的多種任務，探索更多應用場景！
實驗：嘗試不同架構、層組合與技術以提升效能。

> [!NOTE]
> 微調是一個反覆的過程。多嘗試、多學習，並調整你的模型，為你的特定任務達成最佳成果！
