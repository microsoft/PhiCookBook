...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-09T22:38:30+00:00",
  "source_file": "md/03.FineTuning/olive-lab/readme.md",
  "language_code": "nl"
}
-->
# Lab. Optimaliseer AI-modellen voor inferentie op het apparaat

## Introductie

> [!IMPORTANT]
> Voor deze lab is een **Nvidia A10 of A100 GPU** vereist met bijbehorende drivers en CUDA toolkit (versie 12+) geïnstalleerd.

> [!NOTE]
> Dit is een **35 minuten durende** lab die je een praktische introductie geeft in de kernconcepten van het optimaliseren van modellen voor inferentie op het apparaat met behulp van OLIVE.
...
- LoRA-adapters (fijn afgesteld model) genereren voor efficiënte inferentie op het apparaat met de ONNX Runtime.
...
Olive voert een *workflow* uit, een geordende reeks van individuele modeloptimalisatietaken genaamd *passes* - voorbeelden van passes zijn: modelcompressie, grafiekcaptatie, kwantisatie, grafiekoptimalisatie. Elke pass heeft een set parameters die je kunt afstemmen om de beste metrics te bereiken, zoals nauwkeurigheid en latency, die worden geëvalueerd door de respectievelijke evaluator. Olive gebruikt een zoekstrategie waarbij een zoekalgoritme elke pass één voor één of meerdere passes samen automatisch afstemt.

#### Voordelen van Olive

- **Vermindert frustratie en tijd** door trial-and-error handmatig experimenteren met verschillende technieken voor grafiekoptimalisatie, compressie en kwantisatie. Definieer je kwaliteits- en prestatie-eisen en laat Olive automatisch het beste model voor je vinden.
- **Meer dan 40 ingebouwde componenten** voor modeloptimalisatie die geavanceerde technieken in kwantisatie, compressie, grafiekoptimalisatie en fijn afstemmen omvatten.
...
- Workflows opbouwen met YAML/JSON om modeloptimalisatie en deployment taken te orkestreren.
...
> [!NOTE]
> Zorg dat je je Azure AI Hub en Project hebt ingericht en je A100 compute hebt ingesteld zoals in Lab 1.
...
1. Volg de instructies op het scherm om verbinding te maken met de Compute. Hierbij selecteer je je Azure Subscription, Resource Group, Project en de Compute naam die je in Lab 1 hebt ingesteld.
...
In VS Code open je een nieuwe terminal met **Ctrl+J** en clone je deze repo:
...
> [!NOTE]
> Bij het inloggen wordt gevraagd je subscription te selecteren. Zorg dat je de subscription kiest die voor deze lab is opgegeven.
...
1. **Bekijk de data:** In dit voorbeeld ga je het Phi-3.5-Mini model fijn afstemmen zodat het gespecialiseerd is in het beantwoorden van reisgerelateerde vragen. De code hieronder toont de eerste records van de dataset, die in JSON lines formaat zijn:

    ```bash
    head data/data_sample_travel.jsonl
    ```

1. **Kwantiseren van het model:** Voordat je het model traint, kwantiseer je het eerst met het volgende commando dat gebruikmaakt van een techniek genaamd Active Aware Quantization (AWQ) +++https://arxiv.org/abs/2306.00978+++. AWQ kwantiseert de gewichten van een model door rekening te houden met de activaties die tijdens inferentie worden geproduceerd. Dit betekent dat het kwantisatieproces de daadwerkelijke datadistributie in de activaties meeneemt, wat leidt tot een betere behoud van modelnauwkeurigheid vergeleken met traditionele methoden voor gewichtskwantisatie.
...
1. **Train the model:** Next, the `olive finetune` commando stemt het gekwantiseerde model fijn af. Het kwantiseren van het model *voor* het fijn afstemmen in plaats van erna geeft een betere nauwkeurigheid omdat het fijn afstemmen een deel van het verlies door kwantisatie herstelt.
...
Om de inferentie van het model te testen, maak je een Python-bestand in je map genaamd **app.py** en kopieer je de volgende code:
...
To find your resource group `"resourceGroup"` en Azure AI Projectnaam bij, en voer dan het commando uit
...
Vervang de `{}` placeholders door de naam van je resource group en Azure AI Projectnaam.
...
Je kunt dan je geüploade model zien en je model deployen op https://ml.azure.com/model/list
