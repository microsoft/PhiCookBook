...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-09T22:12:00+00:00",
...
[AI Toolkit voor VS Code](https://github.com/microsoft/vscode-ai-toolkit/tree/main) brengt verschillende modellen samen uit de Azure AI Studio Catalog en andere catalogi zoals Hugging Face. De toolkit vereenvoudigt de veelvoorkomende ontwikkeltaken voor het bouwen van AI-apps met generatieve AI-tools en modellen door middel van:
- Aan de slag met modelontdekking en playground.
- Model fine-tuning en inferentie met lokale computerbronnen.
- Externe fine-tuning en inferentie met Azure-bronnen.
...
- [**\[Private Preview\]** Externe ontwikkeling](../../../../md/03.FineTuning)
  ...
  - [\[Optioneel\] Voeg Huggingface token toe aan het Azure Container App geheim](../../../../md/03.FineTuning)
...
1. Zorg ervoor dat de NVIDIA-driver geïnstalleerd is op de host.
2. Voer `huggingface-cli login` uit als je HF gebruikt voor datasetgebruik.
3. `Olive` geeft uitleg over belangrijke instellingen die het geheugenverbruik aanpassen.

### Conda activeren
Omdat we de WSL-omgeving gebruiken en deze gedeeld is, moet je de conda-omgeving handmatig activeren. Na deze stap kun je fine-tuning of inferentie uitvoeren.
...
Zodra de werkruimte geopend is in een dev container, open je een terminal (de standaardlocatie is de projectroot) en voer je het onderstaande commando uit om een LLM te fine-tunen op de geselecteerde dataset.

```bash
python finetuning/invoke_olive.py 
```

Checkpoints en het eindmodel worden opgeslagen in `models` folder.
...
Once deployment is successfully completed, you can access the inference API by clicking on the "*Go to Inference Endpoint*" button displayed in the VSCode notification. Or, the web API endpoint can be found under `ACA_APP_ENDPOINT` in `./infra/inference.config.json` en het outputpaneel te gebruiken. Je bent nu klaar om het model te evalueren via deze endpoint.
