...
[AI Toolkit for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.windows-ai-studio) 通过整合来自 Microsoft Foundry Catalog 及 Hugging Face 等目录的前沿 AI 开发工具和模型，简化了生成式 AI 应用的开发。你可以浏览由 GitHub Models 和 Microsoft Foundry Model Catalogs 提供支持的 AI 模型目录，下载到本地或远程，进行微调、测试并在应用中使用。
