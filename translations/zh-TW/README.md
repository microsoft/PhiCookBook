...
[![在 Dev Containers 打開](https://img.shields.io/static/v1?style=for-the-badge&label=Dev%20Containers&message=Open&color=blue&logo=visualstudiocode)](https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=https://github.com/microsoft/phicookbook)
...
Phi 目前是最強大且具成本效益的小型語言模型（SLM），在多語言、推理、文字/聊天生成、程式編碼、影像、音訊及其他情境中表現卓越。

您可以將 Phi 部署到雲端或邊緣設備，並且可以使用有限的運算能力輕鬆構建生成式 AI 應用程式。

請依序完成以下步驟以開始使用這些資源：
1. **分叉此版本庫**：點擊 [![GitHub 分叉](https://img.shields.io/github/forks/microsoft/phicookbook.svg?style=social&label=Fork)](https://GitHub.com/microsoft/phicookbook/network/?WT.mc_id=aiml-137032-kinfeylo)
2. **克隆此版本庫**： `git clone https://github.com/microsoft/PhiCookBook.git`
3. [**加入微軟 AI Discord 社群，與專家及開發者交流**](https://discord.com/invite/ByRwuEEgH4?WT.mc_id=aiml-137032-kinfeylo)

![cover](../../translated_images/zh-TW/cover.eb18d1b9605d754b.webp)

### 🌐 多語言支援

#### 可透過 GitHub Action 支援（自動且始終保持更新）
...
> 本版本庫包含 50 多種語言翻譯，會顯著增加下載大小。若想不含翻譯檔案的克隆，請使用稀疏簽出：
> ```bash
> git clone --filter=blob:none --sparse https://github.com/microsoft/PhiCookBook.git
> cd PhiCookBook
> git sparse-checkout set --no-cone '/*' '!translations' '!translated_images'
> ```
> 這讓您獲得完成課程所需的所有內容，下載速度也更快。
...
- 推論 Phi 家族
    ...
    - [在本地伺服器中推論 Phi](./md/01.Introduction/03/Local_Server_Inference.md)
...
- Phi 應用程式開發範例
  - 文字及聊天應用
    - Phi-4 範例 🆕
      ...
      - [使用 Sementic Kernel 與 Phi-4 ONNX 的聊天 .NET 主控台應用](../../md/04.HOL/dotnet/src/LabsPhi4-Chat-02SK)
    - Phi-3 / 3.5 範例
      - [使用 Phi3、ONNX Runtime Web 和 WebGPU 的瀏覽器本地聊天機器人](https://github.com/microsoft/onnxruntime-inference-examples/tree/main/js/chat)
      ...
      - [WinUI3 應用程式使用 Phi-3 mini-4k-instruct-onnx](https://github.com/microsoft/Phi3-Chat-WinUI3-Sample/)
      ...
      - [在 Azure AI Foundry 評估微調過的 Phi-3 / Phi-3.5 模型，聚焦於微軟的責任 AI 原則](./md/02.Application/01.TextAndChat/Phi3/E2E_Phi-3-Evaluation_AIFoundry.md)
      ...
      - [在 Windows GPU 使用 Phi-3.5-Instruct ONNX 建立 Prompt flow 解決方案](./md/02.Application/01.TextAndChat/Phi3/UsingPromptFlowWithONNX.md)
      - [使用 Microsoft Phi-3.5 tflite 建立 Android 應用程式](./md/02.Application/01.TextAndChat/Phi3/UsingPhi35TFLiteCreateAndroidApp.md)
      - [Q&A .NET 範例使用本地 ONNX Phi-3 模型與 Microsoft.ML.OnnxRuntime](../../md/04.HOL/dotnet/src/LabsPhi301)
  ...
  - Azure AI 推論 SDK 程式碼範例 
    ...
    - Phi-3 / 3.5 範例
      - [用 Microsoft Phi-3 家族建立專屬 Visual Studio Code GitHub Copilot Chat](./md/02.Application/02.Code/Phi3/VSCodeExt/README.md)
      - [用 Phi-3.5 及 GitHub 模型建立專屬 Visual Studio Code Chat Copilot Agent](/md/02.Application/02.Code/Phi3/CreateVSCodeChatAgentWithGitHubModels.md)
  ...
  - 展示範例
      - [Hugging Face Spaces 上的 Phi-4-mini 展示](https://huggingface.co/spaces/microsoft/phi-4-mini?WT.mc_id=aiml-137032-kinfeylo)
      - [Hugginge Face Spaces 上的 Phi-4-multimodal 展示](https://huggingface.co/spaces/microsoft/phi-4-multimodal?WT.mc_id=aiml-137032-kinfeylo)
  - 視覺範例
    - Phi-4 範例 🆕
    ...
    - Phi-3 / 3.5 範例
      ...
      - [📓][Phi-3.5 視覺多框架或多影像範例](./md/02.Application/04.Vision/Phi3/phi3-vision-demo.ipynb)
      - [Phi-3 視覺本地 ONNX 模型，使用 Microsoft.ML.OnnxRuntime .NET](../../md/04.HOL/dotnet/src/LabsPhi303)
      - [使用選單的 Phi-3 視覺本地 ONNX 模型，使用 Microsoft.ML.OnnxRuntime .NET](../../md/04.HOL/dotnet/src/LabsPhi304)
  ...
  - 音訊範例
    - Phi-4 範例 🆕
      ...
      - [.NET 主控台應用使用 Phi-4-multimodal 音訊分析音檔並生成逐字稿](../../md/04.HOL/dotnet/src/LabsPhi4-MultiModal-02Audio)

  - MOE 範例
    - Phi-3 / 3.5 範例
      - [📓] [Phi-3.5 專家混合模型（MoEs）社群媒體範例](./md/02.Application/06.MoE/Phi3/phi3_moe_demo.ipynb)
      - [📓] [使用 NVIDIA NIM Phi-3 MOE、Azure AI Search 與 LlamaIndex 建立檢索增強生成（RAG）管線](./md/02.Application/06.MoE/Phi3/azure-ai-search-nvidia-rag.ipynb)
      - 
  - 函式調用範例
    - Phi-4 範例 🆕
      -  [📓] [使用 Phi-4-mini 進行函式調用](./md/02.Application/07.FunctionCalling/Phi4/FunctionCallingBasic/README.md)
      -  [📓] [使用函式調用建立多代理人，搭配 Phi-4-mini](./md/02.Application/07.FunctionCalling/Phi4/Multiagents/Phi_4_mini_multiagent.ipynb)
      -  [📓] [使用函式調用結合 Ollama](./md/02.Application/07.FunctionCalling/Phi4/Ollama/ollama_functioncalling.ipynb)
      -  [📓] [使用函式調用結合 ONNX](./md/02.Application/07.FunctionCalling/Phi4/ONNX/onnx_parallel_functioncalling.ipynb)
  - 多模態混合範例
    - Phi-4 範例 🆕
      -  [📓] [使用 Phi-4-multimodal 擔任科技記者](./md/02.Application/08.Multimodel/Phi4/TechJournalist/phi_4_mm_audio_text_publish_news.ipynb)
      - [.NET 主控台應用使用 Phi-4-multimodal 來分析圖片](../../md/04.HOL/dotnet/src/LabsPhi4-MultiModal-01Images)
...
- 學術研究論文與出版品
  - [教科書是你所需要的一切 II：phi-1.5 技術報告](https://arxiv.org/abs/2309.05463)
  - [Phi-3 技術報告：在您手機本地的高效能語言模型](https://arxiv.org/abs/2404.14219)
  ...
  - [(WhyPHI) 針對多選題回答調整 PHI-3：方法論、結果與挑戰](https://arxiv.org/abs/2501.01588)
...
您可以學習如何使用 Microsoft Phi 以及如何在不同硬體設備中建置端到端解決方案。若要親自體驗 Phi，請先使用 [Azure AI Foundry Azure AI Model Catalog](https://aka.ms/phi3-azure-ai) 玩模型並針對您的場景自訂 Phi，更多細節請參考入門教學 [Azure AI Foundry](/md/02.QuickStart/AzureAIFoundry_QuickStart.md)

**Playground**  
每個模型都有專屬的遊戲場地讓您測試模型 [Azure AI Playground](https://aka.ms/try-phi3)。

### GitHub 上的 Phi 模型

您可以學習如何使用 Microsoft Phi 以及如何在不同硬體設備中建置端到端解決方案。若要親自體驗 Phi，請先使用 [GitHub Model Catalog](https://github.com/marketplace/models?WT.mc_id=aiml-137032-kinfeylo) 玩模型並針對您的場景自訂 Phi，更多細節請參考入門教學 [GitHub Model Catalog](/md/02.QuickStart/GitHubModel_QuickStart.md)

**Playground**  
每個模型都有專屬的[測試遊戲場地](/md/02.QuickStart/GitHubModel_QuickStart.md)。
...
微軟致力於幫助客戶負責任地使用我們的 AI 產品，分享我們的經驗，並透過「透明度說明」和「影響評估」等工具建立基於信任的夥伴關係。這些資源多數可在 [https://aka.ms/RAI](https://aka.ms/RAI) 找到。  
微軟負責任 AI 的方法建立在我們公平性、可靠性與安全性、隱私與安全、包容性、透明性及問責制等 AI 原則之上。

大型自然語言、影像及語音模型——與本範例中使用的相同——可能會出現不公平、不可靠或冒犯性的行為，進而造成傷害。請參閱 [Azure OpenAI 服務透明度說明](https://learn.microsoft.com/legal/cognitive-services/openai/transparency-note?tabs=text) 以了解風險與限制。

推薦的風險緩解做法是在您的架構中包含安全系統，用以偵測並防止有害行為。[Azure AI Content Safety](https://learn.microsoft.com/azure/ai-services/content-safety/overview) 提供獨立防護層，能偵測應用程式與服務中用戶生成及 AI 生成的有害內容。Azure AI Content Safety 包含文字與影像 API，可讓您檢測有害資料。在 Azure AI Foundry 中，Content Safety 服務允許您檢視、探索並試用檢測不同模態有害內容的範例程式碼。以下 [快速入門文件](https://learn.microsoft.com/azure/ai-services/content-safety/quickstart-text?tabs=visual-studio%2Clinux&pivots=programming-language-rest) 將引導您完成向該服務提出請求。
另一個需要考慮的方面是整體應用程式效能。對於多模態和多模型的應用程式，我們認為效能是指系統能依照您和使用者的期望進行，包括不產生有害輸出。評估整體應用程式的效能使用[效能與品質以及風險與安全評估器](https://learn.microsoft.com/azure/ai-studio/concepts/evaluation-metrics-built-in)非常重要。您還可以使用[自訂評估器](https://learn.microsoft.com/azure/ai-studio/how-to/develop/evaluate-sdk#custom-evaluators)來建立和評估。

您可以在開發環境中使用[Azure AI 評估 SDK](https://microsoft.github.io/promptflow/index.html)來評估您的 AI 應用程式。無論是給定測試資料集或目標，您的生成式 AI 應用程式產生結果都可以用內建評估器或您選擇的自訂評估器進行量化測量。若要開始使用 Azure AI 評估 SDK 評估您的系統，您可以參考[快速入門指南](https://learn.microsoft.com/azure/ai-studio/how-to/develop/flow-evaluate-sdk)。執行評估後，您可以在[Azure AI Foundry 中視覺化結果](https://learn.microsoft.com/azure/ai-studio/how-to/evaluate-flow-results)。

## 商標

本專案可能包含專案、產品或服務的商標或標誌。Microsoft 商標或標誌的授權使用必須遵守並遵循[Microsoft 商標與品牌指南](https://www.microsoft.com/legal/intellectualproperty/trademarks/usage/general)。在本專案的修改版本中使用 Microsoft 商標或標誌不得造成混淆或暗示 Microsoft 的贊助。任何第三方商標或標誌的使用均須遵守該第三方的政策。

## 尋求協助

如果在開發 AI 應用時遇到困難或有任何問題，歡迎加入：

[![Azure AI Foundry Discord](https://img.shields.io/badge/Discord-Azure_AI_Foundry_Community_Discord-blue?style=for-the-badge&logo=discord&color=5865f2&logoColor=fff)](https://aka.ms/foundry/discord)

如果您在開發期間有產品反饋或錯誤，請訪問：
...
**免責聲明**：  
本文件係使用人工智慧翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。雖然我們致力於確保翻譯的準確性，但請注意，自動翻譯可能包含錯誤或不準確之處。原始文檔的母語版本應視為權威依據。對於重要資訊，建議採用專業人工翻譯。我們不對因使用此翻譯而產生的任何誤解或誤釋負責。
