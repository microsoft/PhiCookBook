...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T15:49:55+00:00",
...
![Olive Flow](../../../../../translated_images/olive-flow.a47985655a756dcba73521511ea42eef359509a3a33cbd4b9ac04ba433287b80.bn.png)
...
Olive একটি *ওয়ার্কফ্লো* চালায়, যা একটির পর একটি সাজানো মডেল অপ্টিমাইজেশন টাস্কের সিকোয়েন্স, যাদের *পাস* বলা হয় - উদাহরণস্বরূপ: মডেল কম্প্রেশন, গ্রাফ ক্যাপচার, কোয়ান্টাইজেশন, গ্রাফ অপ্টিমাইজেশন। প্রতিটি পাসের কিছু প্যারামিটার থাকে যা সেরা মেট্রিক্স (যেমন সঠিকতা ও লেটেন্সি) অর্জনের জন্য টিউন করা যায়, যা সংশ্লিষ্ট ইভ্যালুয়েটর দ্বারা মূল্যায়ন করা হয়। Olive একটি সার্চ স্ট্র্যাটেজি ব্যবহার করে যা প্রতিটি পাস একে একে বা একসাথে টিউন করার জন্য সার্চ অ্যালগরিদম প্রয়োগ করে।

#### Olive-এর সুবিধাসমূহ

- গ্রাফ অপ্টিমাইজেশন, কম্প্রেশন এবং কোয়ান্টাইজেশনের বিভিন্ন পদ্ধতির জন্য ম্যানুয়াল ট্রায়াল-এন্ড-এরর পরীক্ষার সময় ও হতাশা কমায়। আপনার গুণগত মান ও কর্মক্ষমতার সীমা নির্ধারণ করুন, Olive স্বয়ংক্রিয়ভাবে সেরা মডেল খুঁজে দেবে।
...
5. সংযোগ সফল হলে Visual Code-এর **বাম নিচে** `><Azure ML: Compute Name` দেখাবে।
...
> [!NOTE]
> সমস্ত ডিপেন্ডেন্সি ইনস্টল করতে প্রায় ৫ মিনিট সময় লাগবে।

এই ল্যাবে আপনি মডেল ডাউনলোড ও আপলোড করবেন Azure AI Model ক্যাটালগে। ক্যাটালগ অ্যাক্সেসের জন্য Azure-এ লগইন করতে হবে:
...
2. **মডেল কোয়ান্টাইজ করুন:** মডেল ট্রেনিংয়ের আগে, নিচের কমান্ড দিয়ে Active Aware Quantization (AWQ) +++https://arxiv.org/abs/2306.00978+++ পদ্ধতি ব্যবহার করে কোয়ান্টাইজেশন করবেন। AWQ মডেলের ওজন কোয়ান্টাইজ করে ইনফারেন্স চলাকালীন উৎপন্ন অ্যাক্টিভেশন বিবেচনা করে। এর ফলে কোয়ান্টাইজেশন প্রক্রিয়া অ্যাক্টিভেশনের প্রকৃত ডেটা বিতরণ বিবেচনা করে, যা প্রচলিত ওজন কোয়ান্টাইজেশন পদ্ধতির তুলনায় মডেলের সঠিকতা ভালোভাবে সংরক্ষণ করে।
    ...
    এই ল্যাবে আমরা Hugging Face থেকে মডেল ইনপুট দেখাচ্ছি (যেমন: `microsoft/Phi-3.5-mini-instruct`)। তবে Olive Azure AI ক্যাটালগ থেকে মডেল ইনপুট করতেও দেয়, যেখানে `model_name_or_path` আর্গুমেন্টটি Azure AI অ্যাসেট আইডিতে (যেমন: `azureml://registries/azureml/models/Phi-3.5-mini-instruct/versions/4`) আপডেট করতে হবে।

3. **মডেল ট্রেন করুন:** এরপর, `olive finetune` কমান্ড কোয়ান্টাইজড মডেল ফাইন-টিউন করবে। কোয়ান্টাইজেশনের পরে নয়, আগে কোয়ান্টাইজ করে ফাইন-টিউন করলে সঠিকতা ভালো হয় কারণ ফাইন-টিউনিং কোয়ান্টাইজেশন থেকে কিছু ক্ষতি পুনরুদ্ধার করে।
...
4. **অপ্টিমাইজ করুন:** মডেল ট্রেনিংয়ের পর, Olive-এর `auto-opt` কমান্ড ব্যবহার করে মডেল অপ্টিমাইজ করুন, যা ONNX গ্রাফ ক্যাপচার করে এবং মডেল পারফরম্যান্স উন্নত করতে CPU-এর জন্য মডেল কম্প্রেশন ও ফিউশন স্বয়ংক্রিয়ভাবে করে। উল্লেখ্য, আপনি `--device` এবং `--provider` আর্গুমেন্ট আপডেট করে NPU বা GPU-এর জন্যও অপ্টিমাইজ করতে পারেন, কিন্তু এই ল্যাবের জন্য CPU ব্যবহার করা হবে।
...
মডেল ইনফারেন্স পরীক্ষা করতে, আপনার ফোল্ডারে **app.py** নামে একটি পাইথন ফাইল তৈরি করুন এবং নিচের কোড কপি-পেস্ট করুন:
