...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-09T09:47:01+00:00",
...
[AI Toolkit for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.windows-ai-studio) поједностављује развој генеративних AI апликација тако што окупља најсавременије алате и моделе за развој AI из Azure AI Foundry каталога и других каталога као што је Hugging Face. Можете прегледати каталог AI модела који покрећу GitHub Models и Azure AI Foundry Model каталози, преузимати их локално или на даљину, фино подешавати, тестирати и користити у вашој апликацији.

AI Toolkit Preview ради локално. Локални inference или фино подешавање зависи од изабраног модела, можда ће вам требати GPU као што је NVIDIA CUDA GPU. Можете директно покретати GitHub Models и са AITK.
...
Ако нисте упознати са инсталирањем VS Code екстензија, пратите ове кораке:

### Пријава

1. У Activity Bar у VS Code-у изаберите **Extensions**
1. У пољу за претрагу Extensions унесите "AI Toolkit"
1. Изаберите "AI Toolkit for Visual Studio code"
1. Изаберите **Install**

Сада сте спремни да користите екстензију!

Бићете затражени да се пријавите на GitHub, па кликните на "Allow" да наставите. Бићете преусмерени на GitHub страницу за пријаву.

Пријавите се и пратите кораке у процесу. Након успешног завршетка, вратићете се у VS Code.

Када се екстензија инсталира, видећете AI Toolkit иконицу у вашем Activity Bar-у.

Хајде да истражимо доступне опције!
...
налазе се у одељку Resources. Да бисте започели, изаберите **Model Catalog**.

### Преузмите модел из каталога

Када покренете AI Toolkit из VS Code бочне траке, можете изабрати следеће опције:
...
- Тестирајте inference модела у **Model Playground**
...
> **GPU у односу на CPU**
>
> Примећујете да картице модела приказују величину модела, платформу и тип акцелератора (CPU, GPU). За оптималан рад на **Windows уређајима који имају бар један GPU**, изаберите верзије модела које су намењене само Windows-у.
>
...
>
>Да бисте проверили да ли имате GPU на вашем Windows уређају, отворите **Task Manager** и изаберите картицу **Performance**. Ако имате GPU(е), биће наведени под именима као што су "GPU 0" или "GPU 1".

### Покрените модел у playground-у

Када су сви параметри подешени, кликните на **Generate Project**.
...
Ово вам омогућава да тестирате вашу апликацију локално без ослањања на cloud AI модел сервис. На пример, следећа JSON датотека показује како да конфигуришете тело захтева:
...
Додајте [Azure OpenAI клијент библиотеку за .NET](https://www.nuget.org/packages/Azure.AI.OpenAI/) у ваш пројекат преко NuGet-а:

```bash
dotnet add {project_name} package Azure.AI.OpenAI --version 1.0.0-beta.17
```

Додајте C# фајл под именом **OverridePolicy.cs** у ваш пројекат и налепите следећи код:
...
- Започните са откривањем модела и playground-ом.
- Фино подешавање и inference модела користећи локалне рачунарске ресурсе.
- Удаљено фино подешавање и inference користећи Azure ресурсе

[Фино подешавање са AI Toolkit-ом](../../03.FineTuning/Finetuning_VSCodeaitoolkit.md)

## AI Toolkit Q&A ресурси

Погледајте нашу [Q&A страницу](https://github.com/microsoft/vscode-ai-toolkit/blob/main/archive/QA.md) за најчешће проблеме и њихова решења
