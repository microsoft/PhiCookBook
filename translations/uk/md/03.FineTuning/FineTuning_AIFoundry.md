...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-09T19:03:34+00:00",
...
Ви можете видалити тонко налаштовану модель зі списку моделей для тонкого налаштування в [Azure AI Foundry](https://ai.azure.com) або зі сторінки деталей моделі. Виберіть модель для видалення на сторінці Fine-tuning, а потім натисніть кнопку Delete, щоб видалити модель.
...
Виберіть відповідну функцію втрат для вашого завдання (наприклад, крос-ентропію).
...
- Завантаження попередньо навченого контролю: Завантажте контрольну точку Phi-3 Mini.
...
Використовуйте метрики, такі як точність, F1-міра або перплексія, для оцінки продуктивності моделі.

## Збереження тонко налаштованої моделі

**Контрольна точка**  
Збережіть контрольну точку тонко налаштованої моделі для подальшого використання.

## Розгортання

- Розгортання як веб-сервіс: Розгорніть тонко налаштовану модель як веб-сервіс у Azure AI Foundry.
- Тестування кінцевої точки: Надішліть тестові запити до розгорнутої кінцевої точки, щоб перевірити її функціональність.

## Ітерації та покращення

Ітеруйте: Якщо продуктивність не задовольняє, коригуйте гіперпараметри, додавайте більше даних або тонко налаштовуйте модель на додаткові епохи.
...
Експериментуйте: Спробуйте різні архітектури, комбінації шарів і техніки для покращення продуктивності.
