...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T16:10:47+00:00",
...
Olive（*O*NNX *live*）是一套模型優化工具包，搭配 CLI，讓你能夠為 ONNX runtime +++https://onnxruntime.ai+++ 提供高品質且高效能的模型。

![Olive 流程](../../../../../translated_images/zh-TW/olive-flow.c4f76d9142c579b2.webp)
...
- **減少手動嘗試錯誤的挫折與時間**，無需反覆實驗不同的圖形優化、壓縮和量化技術。定義你的品質與效能限制，讓 Olive 自動為你找到最佳模型。
- **內建 40 多種模型優化元件**，涵蓋量化、壓縮、圖形優化和微調的尖端技術。
- **易用的 CLI**，支援常見的模型優化任務，例如 olive quantize、olive auto-opt、olive finetune。
- 內建模型封裝與部署功能。
- 支援生成 **多 LoRA 服務** 的模型。
...
本實驗室將下載並上傳模型至 Azure AI 模型目錄。為了存取模型目錄，你需要登入 Azure：
...
2. **量化模型：** 在訓練模型前，先使用 Active Aware Quantization (AWQ) +++https://arxiv.org/abs/2306.00978+++ 技術進行量化。AWQ 會考慮推論時產生的激活值，根據激活的實際資料分布來量化權重，較傳統權重量化方法能更好地保留模型準確度。
...
3. **訓練模型：** 接著，使用 `olive finetune` 指令微調量化後的模型。先量化再微調比先微調後量化能取得更佳準確度，因為微調過程能彌補部分量化造成的損失。
...
4. **優化模型：** 模型訓練完成後，使用 Olive 的 `auto-opt` 指令優化模型，該指令會捕捉 ONNX 圖形並自動執行多項優化，提升 CPU 上的模型效能，包含模型壓縮與融合。你也可以透過更新 `--device` 和 `--provider` 參數來優化其他裝置（如 NPU 或 GPU），但本實驗室以 CPU 為例。
