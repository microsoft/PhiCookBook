...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T10:14:36+00:00",
...
![Olive 流程圖](../../../../../translated_images/olive-flow.5daf97340275f8b61397e91430ff02724a2547937b352e7fdfc2f669c56dcd35.tw.png)
...
- **減少手動嘗試不同圖形優化、壓縮和量化技術的挫折與時間。** 定義你的品質與效能限制，讓 Olive 自動為你找到最佳模型。
...
在 VS Code 中，使用 **Ctrl+J** 開啟新終端機，並克隆此儲存庫：
...
或者，你也可以透過選單 **File** > **Open Folder** 開啟資料夾。

### 步驟 3：安裝相依套件

在 VS Code 中的 Azure AI 計算實例終端機（提示：**Ctrl+J**）執行以下指令安裝相依套件：
...
在 VS Code 中的 Azure AI 計算實例終端機（提示：**Ctrl+J**）確保已啟動 `olive-ai` conda 環境：
...
2. **量化模型：** 在訓練模型前，先使用以下指令進行量化，採用一種稱為 Active Aware Quantization (AWQ) +++https://arxiv.org/abs/2306.00978+++ 的技術。AWQ 會根據推論時產生的激活值來量化模型權重，這表示量化過程會考慮激活值的實際資料分布，較傳統權重量化方法能更好地保留模型準確度。
    ...
    本實驗室示範如何從 Hugging Face 輸入模型（例如：`microsoft/Phi-3.5-mini-instruct`），但 Olive 也允許你從 Azure AI 目錄輸入模型，只需將 `model_name_or_path` 參數更新為 Azure AI 資產 ID（例如：`azureml://registries/azureml/models/Phi-3.5-mini-instruct/versions/4`）。

3. **訓練模型：** 接著，使用 `olive finetune` 指令微調量化後的模型。先量化再微調比起先微調再量化能獲得更佳準確度，因為微調過程能補償部分量化帶來的損失。
...
4. **優化模型：** 模型訓練完成後，使用 Olive 的 `auto-opt` 指令優化模型，該指令會捕捉 ONNX 圖形並自動執行多項優化，提升模型在 CPU 上的效能，包含模型壓縮與融合。你也可以透過更新 `--device` 和 `--provider` 參數來針對 NPU 或 GPU 進行優化，但本實驗室以 CPU 為例。
...
為測試模型推論，在資料夾中建立一個名為 **app.py** 的 Python 檔案，並貼上以下程式碼：
...
若要查詢你的資源群組 `"resourceGroup"` 與 Azure AI 專案名稱，請執行：
