...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T20:48:20+00:00",
...
- 使用容器映像管理模型檔案。提供 HTTP 伺服器以使用模型庫進行推論呼叫。
...
可以透過執行以下指令追蹤 workspace 狀態。當 WORKSPACEREADY 欄位顯示為 `True` 時，表示模型已成功部署。
...
接著，可以找到推論服務的叢集 IP，並使用臨時的 `curl` Pod 在叢集中測試服務端點。
...
## 使用 adapters 快速開始推論 Phi-3

安裝 Kaito 後，可以嘗試以下指令啟動推論服務。

[帶 adapters 的 Phi-3 推論範例程式碼](https://github.com/Azure/kaito/blob/main/examples/inference/kaito_workspace_phi_3_with_adapters.yaml)
