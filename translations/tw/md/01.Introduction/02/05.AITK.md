...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T19:22:47+00:00",
...
[AI Toolkit for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.windows-ai-studio) 透過整合來自 Azure AI Foundry Catalog 及其他目錄如 Hugging Face 的先進 AI 開發工具和模型，簡化生成式 AI 應用程式的開發。您可以瀏覽由 GitHub Models 和 Azure AI Foundry Model Catalogs 提供的 AI 模型目錄，將模型下載到本地或遠端，進行微調、測試並在您的應用程式中使用。
...
- 在 Windows 和 Linux 上進行微調時，您需要 Nvidia GPU。此外，**Windows** 需要安裝 Ubuntu 18.4 或更新版本的 Linux 子系統。請參考 [了解如何安裝 Windows 子系統 for Linux](https://learn.microsoft.com/windows/wsl/install) 及 [更改預設發行版](https://learn.microsoft.com/windows/wsl/install#change-the-default-linux-distribution-installed)。
...
這些功能都在 Resources 區域中。要開始使用，請選擇 **Model Catalog**。
...
> **GPU 與 CPU**
>
> 您會注意到模型卡片會顯示模型大小、平台及加速器類型（CPU、GPU）。若您使用的 **Windows 裝置至少有一張 GPU**，建議選擇僅針對 Windows 優化的模型版本，以獲得最佳效能。
>
> 這樣可以確保模型針對 DirectML 加速器進行優化。
>
> 模型名稱格式為
>
...
這讓您能在本地測試應用程式，而不必依賴雲端 AI 模型服務。例如，以下 JSON 檔案示範如何設定請求主體：
...
使用 NuGet 將 [Azure OpenAI 用戶端函式庫 for .NET](https://www.nuget.org/packages/Azure.AI.OpenAI/) 加入您的專案：
