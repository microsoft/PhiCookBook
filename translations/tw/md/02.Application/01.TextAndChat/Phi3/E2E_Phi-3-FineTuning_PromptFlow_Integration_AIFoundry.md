...
![Phi-3-FineTuning_PromptFlow_Integration Overview.](../../../../../../translated_images/00-01-architecture.198ba0f1ae6d841a.tw.png)
...
1. 在入口網站頁面頂端的 **搜尋列** 輸入 *azure machine learning*，並從出現的選項中選擇 **Azure Machine Learning**。
    ...
    ![Type azure machine learning.](../../../../../../translated_images/01-01-type-azml.acae6c5455e67b4b.tw.png)

2. 從導覽選單中選擇 **+ Create**。

3. 從導覽選單中選擇 **New workspace**。

    ![Select new workspace.](../../../../../../translated_images/01-02-select-new-workspace.cd09cd0ec4a60ef2.tw.png)
...
4. 執行以下操作：
    ...
    ![Fill azure machine learning.](../../../../../../translated_images/01-03-fill-AZML.a1b6fd944be0090f.tw.png)
...
1. 申請 *Standard NCADSA100v4 Family* 配額：
    ...
    - 從導覽選單選擇 **Request quota**。
        ...
        ![Request quota.](../../../../../../translated_images/02-02-request-quota.c0428239a63ffdd5.tw.png)
...
1. 在入口網站頁面頂端的 **搜尋列** 輸入 *managed identities*，並從出現的選項中選擇 **Managed Identities**。
    ...
    ![Type managed identities.](../../../../../../translated_images/03-01-type-managed-identities.24de763e0f1f37e5.tw.png)

1. 選擇 **+ Create**。

    ![Select create.](../../../../../../translated_images/03-02-select-create.92bf8989a5cd98f2.tw.png)
...
1. 執行以下操作：
    ...
    ![Select create.](../../../../../../translated_images/03-03-fill-managed-identities-1.ef1d6a2261b449e0.tw.png)
...
1. 在新增角色指派頁面執行以下操作：
    ...
    ![Fill contributor role.](../../../../../../translated_images/03-04-fill-contributor-role.73990bc6a32e140d.tw.png)
...
1. 在入口網站頁面頂端的 **搜尋列** 輸入 *storage accounts*，並從出現的選項中選擇 **Storage accounts**。
    ...
    ![Type storage accounts.](../../../../../../translated_images/03-05-type-storage-accounts.9303de485e65e1e5.tw.png)
...
1. 執行以下操作以進入新增角色指派頁面：
    ...
    ![Add role.](../../../../../../translated_images/03-06-add-role.353ccbfdcf0789c2.tw.png)
...
1. 在新增角色指派頁面執行以下操作：
    ...
    ![Select managed identity.](../../../../../../translated_images/03-08-select-managed-identity.e80a2aad5247eb25.tw.png)
...
1. 在入口網站頁面頂端的 **搜尋列** 輸入 *container registries*，並從出現的選項中選擇 **Container registries**。
    ...
    ![Type container registries.](../../../../../../translated_images/03-09-type-container-registries.7a4180eb2110e5a6.tw.png)
...
1. 選擇你建立的 *finetune-phi* 資料夾，路徑位於 *C:\Users\yourUserName\finetune-phi*。
    ...
    ![選擇你建立的資料夾。](../../../../../../translated_images/04-01-open-project-folder.f734374bcfd5f9e6.tw.png)

1. 在 Visual Studio Code 左側窗格中，右鍵點擊並選擇 **New File**，建立一個名為 *download_dataset.py* 的新檔案。

    ![建立新檔案。](../../../../../../translated_images/04-02-create-new-file.cf9a330a3a9cff92.tw.png)
...
1. 點選 **+ New**。
    ...
    ![選擇 Compute。](../../../../../../translated_images/06-01-select-compute.a29cff290b480252.tw.png)
...
1. 執行以下設定：
    ...
    ![建立叢集。](../../../../../../translated_images/06-02-create-cluster.f221b65ae1221d4e.tw.png)
...
1. 執行以下設定：
    ...
    ![建立叢集。](../../../../../../translated_images/06-03-create-cluster.4a54ba20914f3662.tw.png)
...
1. 選擇你建立的 Azure Machine Learning 工作區。
    ...
    ![選擇你建立的工作區。](../../../../../../translated_images/06-04-select-workspace.a92934ac04f4f181.tw.png)
...
1. 執行以下操作：
    ...
    ![輸入 phi-3-mini-4k。](../../../../../../translated_images/06-05-type-phi-3-mini-4k.8ab6d2a04418b250.tw.png)

1. 從導覽選單選擇 **Fine-tune**。

    ![選擇微調。](../../../../../../translated_images/06-06-select-fine-tune.2918a59be55dfeec.tw.png)
...
1. 執行以下設定：
    ...
    ![填寫微調頁面。](../../../../../../translated_images/06-07-fill-finetuning.b6d14c89e7c27d0b.tw.png)
...
1. 在本練習中，你已成功使用 Azure Machine Learning 微調 Phi-3 模型。請注意，微調過程可能需要相當長的時間。提交微調工作後，需等待其完成。你可以在 Azure Machine Learning 工作區左側的 Jobs 標籤中監控微調工作的狀態。接下來的系列中，你將部署微調後的模型並與 Prompt flow 整合。
    ...
    ![查看微調工作。](../../../../../../translated_images/06-08-output.2bd32e59930672b1.tw.png)
...
1. 選擇 **From a job output**。
    ...
    ![註冊模型。](../../../../../../translated_images/07-01-register-model.ad1e7cc05e4b2777.tw.png)

1. 選擇你建立的工作。

    ![選擇工作。](../../../../../../translated_images/07-02-select-job.3e2e1144cd6cd093.tw.png)
...
1. 確認已選擇 **Job output**，此選項應自動被選中。
    ...
    ![選擇輸出。](../../../../../../translated_images/07-03-select-output.4cf1a0e645baea1f.tw.png)

2. 點選 **Next**。

3. 點選 **Register**。

    ![點選註冊。](../../../../../../translated_images/07-04-register.fd82a3b293060bc7.tw.png)

4. 你可以透過左側標籤的 **Models** 選單查看已註冊的模型。

    ![已註冊的模型。](../../../../../../translated_images/07-05-registered-model.7db9775f58dfd591.tw.png)
...
1. 從導覽選單選擇 **Real-time endpoints**。
    ...
    ![建立端點。](../../../../../../translated_images/07-06-create-endpoint.1ba865c606551f09.tw.png)

1. 點選 **Create**。

1. 選擇你已註冊的模型。

    ![選擇已註冊的模型。](../../../../../../translated_images/07-07-select-registered-model.29c947c37fa30cb4.tw.png)
...
1. 執行以下設定：
    ...
    ![填寫部署設定。](../../../../../../translated_images/07-08-deployment-setting.43ddc4209e673784.tw.png)
...
1. 選擇你建立的端點。
    ...
    ![選擇端點](../../../../../../translated_images/07-09-check-deployment.325d18cae8475ef4.tw.png)
...
>
> ![設定流量。](../../../../../../translated_images/07-10-set-traffic.085b847e5751ff3d.tw.png)
>
...
1. 從導覽選單中選擇 **+ New hub**。
    ...
    ![Create hub.](../../../../../../translated_images/08-01-create-hub.8f7dd615bb8d9834.tw.png)
...
1. 執行以下操作：
    ...
    ![Fill hub.](../../../../../../translated_images/08-02-fill-hub.c2d3b505bbbdba7c.tw.png)
...
1. 從導覽選單中選擇 **+ New project**。
    ...
    ![Select new project.](../../../../../../translated_images/08-04-select-new-project.390fadfc9c8f8f12.tw.png)

1. 輸入 **Project name**，必須是唯一值。

    ![Create project.](../../../../../../translated_images/08-05-create-project.4d97f0372f03375a.tw.png)
...
1. 從左側標籤選擇 **Endpoints**。
    ...
    ![Select endpoints.](../../../../../../translated_images/08-06-select-endpoints.aff38d453bcf9605.tw.png)

1. 選擇您建立的端點。

    ![Select endpoints.](../../../../../../translated_images/08-07-select-endpoint-created.47f0dc09df2e275e.tw.png)

1. 從導覽選單選擇 **Consume**。

1. 複製您的 **REST endpoint** 和 **Primary key**。

    ![Copy api key and endpoint uri.](../../../../../../translated_images/08-08-copy-endpoint-key.18f934b5953ae8cb.tw.png)
...
1. 選擇 **+ New connection**。
    ...
    ![Select new connection.](../../../../../../translated_images/08-09-select-new-connection.02eb45deadc401fc.tw.png)

1. 從導覽選單選擇 **Custom keys**。

    ![Select custom keys.](../../../../../../translated_images/08-10-select-custom-keys.856f6b2966460551.tw.png)
...
1. 執行以下操作：
    ...
    ![Add connection.](../../../../../../translated_images/08-11-add-connection.785486badb4d2d26.tw.png)
...
1. 從導覽選單選擇 **+ Create**。
    ...
    ![Select Promptflow.](../../../../../../translated_images/08-12-select-promptflow.6f4b451cb9821e5b.tw.png)

1. 從導覽選單選擇 **Chat flow**。

    ![Select chat flow.](../../../../../../translated_images/08-13-select-flow-type.2ec689b22da32591.tw.png)

1. 輸入要使用的 **Folder name**。

    ![Enter name.](../../../../../../translated_images/08-14-enter-name.ff9520fefd89f40d.tw.png)
...
1. 在 Prompt flow 中，執行以下操作以重建現有流程：
    ...
    ![Select raw file mode.](../../../../../../translated_images/08-15-select-raw-file-mode.61d988b41df28985.tw.png)
...
1. 將以下程式碼新增到 *integrate_with_promptflow.py* 檔案中，以在 Prompt flow 中使用自訂 Phi-3 模型。
    ...
    ![Paste prompt flow code.](../../../../../../translated_images/08-16-paste-promptflow-code.a6041b74a7d09777.tw.png)
...
1. 選擇 **Chat input**、**Chat output**，以啟用與模型的對話功能。
    ...
    ![Input Output.](../../../../../../translated_images/08-17-select-input-output.64dbb39bbe59d03b.tw.png)
...
> 重建後的流程應該如下圖所示：
>
> ![Flow example.](../../../../../../translated_images/08-18-graph-example.d6457533952e690c.tw.png)
>
...
1. 選擇 **Start compute sessions** 以啟動 Prompt flow。
    ...
    ![Start compute session.](../../../../../../translated_images/09-01-start-compute-session.a86fcf5be68e386b.tw.png)

1. 選擇 **Validate and parse input** 以更新參數。

    ![Validate input.](../../../../../../translated_images/09-02-validate-input.317c76ef766361e9.tw.png)

1. 選擇 **connection** 的 **Value**，選擇您建立的自訂連線，例如 *connection*。

    ![Connection.](../../../../../../translated_images/09-03-select-connection.99bdddb4b1844023.tw.png)

#### 與您的自訂模型對話

1. 選擇 **Chat**。

    ![Select chat.](../../../../../../translated_images/09-04-select-chat.61936dce6612a1e6.tw.png)

1. 以下是範例結果：現在您可以與自訂 Phi-3 模型對話。建議根據用於微調的資料來提問。

    ![Chat with prompt flow.](../../../../../../translated_images/09-05-chat-with-promptflow.c8ca404c07ab126f.tw.png)
