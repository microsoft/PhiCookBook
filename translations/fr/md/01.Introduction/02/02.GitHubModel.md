...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-07T15:10:04+00:00",
...
Chaque modèle dispose d’un espace de démonstration dédié et d’exemples de code
...
Plusieurs exemples simples sont prêts à être exécutés. Vous les trouverez dans le répertoire samples. Si vous souhaitez passer directement à votre langage préféré, voici les exemples disponibles dans les langues suivantes :
...
Voici quelques extraits de code pour différents cas d’usage. Pour plus d’informations sur Azure AI Inference SDK, consultez la documentation complète et les exemples.

## Configuration

1. Créez un jeton d’accès personnel  
Vous n’avez pas besoin d’attribuer de permissions particulières au jeton. Notez que le jeton sera envoyé à un service Microsoft.
...
Installez Azure AI Inference SDK avec pip (Requis : Python >=3.8) :
...
Cet exemple montre un appel basique à l’API de complétion de chat. Il utilise le point d’accès d’inférence du modèle AI GitHub ainsi que votre jeton GitHub. L’appel est synchrone.
...
Cet exemple illustre une conversation multi-tours avec l’API de complétion de chat. Lors de l’utilisation du modèle pour une application de chat, il faut gérer l’historique de la conversation et envoyer les derniers messages au modèle.
...
Pour une meilleure expérience utilisateur, il est conseillé de diffuser la réponse du modèle afin que le premier token apparaisse rapidement, évitant ainsi d’attendre les réponses longues.
...
Les [limites de débit pour le playground et l’utilisation gratuite de l’API](https://docs.github.com/en/github-models/prototyping-with-ai-models#rate-limits) sont conçues pour vous permettre d’expérimenter avec les modèles et de prototyper votre application IA. Pour un usage au-delà de ces limites, et pour faire évoluer votre application, vous devez provisionner des ressources via un compte Azure, et vous authentifier à partir de là au lieu d’utiliser votre jeton d’accès personnel GitHub. Vous n’avez rien d’autre à modifier dans votre code. Utilisez ce lien pour découvrir comment dépasser les limites du palier gratuit dans Azure AI.
...
GitHub Models utilise Azure AI Content Safety. Ces filtres ne peuvent pas être désactivés dans le cadre de l’expérience GitHub Models. Si vous choisissez d’utiliser les modèles via un service payant, configurez vos filtres de contenu selon vos besoins.
