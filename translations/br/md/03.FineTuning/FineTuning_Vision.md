...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-09T22:00:15+00:00",
  "source_file": "md/03.FineTuning/FineTuning_Vision.md",
  "language_code": "br"
}
-->
# Phi-3.5-vision receita de ajuste fino

Este é o suporte oficial para ajuste fino do Phi-3.5-vision usando as bibliotecas huggingface.  
...
Fornecemos dois scripts de ajuste fino de exemplo, um para DocVQA e outro para classificação de memes ofensivos.
...
Phi-3.5-vision agora suporta oficialmente entradas com múltiplas imagens. Aqui está um exemplo de ajuste fino para NLVR2
...
Dependendo do hardware, os usuários podem escolher diferentes estratégias de ajuste fino. Suportamos ajuste fino completo (com Deepspeed Zero-2) com parâmetros de visão opcionalmente congelados, e LoRA (incluindo QLoRA 4bit).  
Em geral, recomendamos usar ajuste fino completo com flash attention e bf16 sempre que possível.

### guia para converter seu conjunto de dados personalizado para o formato requerido

Usamos um conjunto mínimo de dados para classificação de vídeo (um subconjunto do UCF-101) como exemplo completo para demonstrar como converter seu conjunto de dados personalizado para o formato requerido e ajustar fino o Phi-3.5-vision nele.
...
Note que `conversations` é uma lista, portanto conversas com múltiplas interações podem ser suportadas se tais dados estiverem disponíveis.

## Solicitando cota de GPU no Azure

### Pré-requisitos

Uma conta Azure com o papel de Contributor (ou outro papel que inclua acesso de Contributor).

Se você não tem uma conta Azure, crie uma [conta gratuita antes de começar](https://azure.microsoft.com).

### Solicitar aumento de cota

Você pode enviar uma solicitação para aumento de cota diretamente em My quotas. Siga os passos abaixo para pedir um aumento de cota. Neste exemplo, você pode selecionar qualquer cota ajustável na sua assinatura.
...
Na página My quotas, em Quota name, selecione a cota que deseja aumentar. Certifique-se que a coluna Adjustable mostre Yes para essa cota.
...
Sua solicitação será revisada e você será notificado se ela puder ser atendida. Isso geralmente ocorre em poucos minutos.
...
O ajuste fino completo geralmente oferece o melhor desempenho. Você pode usar o comando a seguir para ajustar fino o Phi-3-V na classificação de memes ofensivos.
...
Ainda é possível ajustar fino completamente o Phi-3-V na classificação de memes ofensivos. Porém, espere um throughput muito menor comparado às GPUs A100 ou H100 devido à falta de suporte a flash attention.  
...
LoRA pode ser sua única opção. Você pode usar o comando a seguir para ajustar fino o Phi-3-V na classificação de memes ofensivos.
...
O benchmark de velocidade é realizado no conjunto de dados DocVQA. O comprimento médio da sequência deste conjunto é 2443.23 tokens (usando `num_crops=16` para o modelo de imagem).
...
- Ainda não suporta salvar checkpoints intermediários e retomar o treinamento.
