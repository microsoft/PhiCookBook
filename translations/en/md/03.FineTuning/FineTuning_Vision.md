...
Depending on your hardware, you can choose different finetuning strategies. We support  
full finetuning (with Deepspeed Zero-2) with optional freezing of vision parameters, and LoRA (including 4bit QLoRA).  
...
We use a minimal video classification dataset (a subset of UCF-101) as an end-to-end example to show how to convert your custom dataset to the required format and finetune Phi-3.5-vision on it.
...
Note that `conversations` is a list, so multi-turn conversations are supported if such data is available.
...
Full finetuning usually delivers the best performance. You can use the following command to finetune Phi-3-V on hateful memes classification.
...
LoRA might be your only option. You can use the following command to finetune Phi-3-V on hateful memes classification.
