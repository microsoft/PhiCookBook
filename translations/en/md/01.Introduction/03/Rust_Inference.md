...
We load the quantized model weights into memory and initialize the Phi-3 model. This step reads the model weights from the `gguf` file and sets up the model for inference on the specified device (CPU in this case).
...
Finally, the generated text is printed as it’s decoded, enabling streamed real-time output.
...
By following these steps, you can perform text generation using the Phi-3 model with Rust and Candle in under 100 lines of code. The code handles model loading, tokenization, and inference, leveraging tensors and logits processing to generate coherent text based on the input prompt.

This console application can run on Windows, Linux, and macOS. Thanks to Rust’s portability, the code can also be adapted into a library to run inside mobile apps (since console apps can’t run there).
