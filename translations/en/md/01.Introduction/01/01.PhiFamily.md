...
|Need a model that simply summarizes a thread of messages|Conversation Summarization|Phi-3 / 3.5 text model|Deciding factor here is that the customer has a well defined and straightforward language task|
...
|Wants to build a copilot for their employees|RAG, multiple domain, complex and open ended|GPT models + Phi Family |Open ended scenario, needs broader world knowledge, hence a larger model is more suited. You need to chunk the knowledge content, maybe SLM is good for you |
