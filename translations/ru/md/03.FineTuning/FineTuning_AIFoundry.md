# Тонкая настройка Phi-3 с помощью Microsoft Foundry

Давайте рассмотрим, как выполнить тонкую настройку языковой модели Phi-3 Mini от Microsoft с помощью Microsoft Foundry. Тонкая настройка позволяет адаптировать Phi-3 Mini под конкретные задачи, делая модель более мощной и контекстно ориентированной.
...
- Для доступа к операциям в Microsoft Foundry используется управление доступом на основе ролей Azure (Azure RBAC). Чтобы выполнить шаги из этой статьи, ваша учетная запись должна иметь роль __Azure AI Developer__ в группе ресурсов.
...
Microsoft Foundry позволяет адаптировать крупные языковые модели под ваши собственные данные с помощью процесса, известного как тонкая настройка. Тонкая настройка дает значительные преимущества, позволяя настраивать и оптимизировать модель для конкретных задач и приложений. Это улучшает производительность, снижает затраты, уменьшает задержки и обеспечивает более точные результаты.
...
2. Нажмите **+New project** для создания нового проекта в Microsoft Foundry.
...
6. Выполните следующие действия:
    ...
    - Выберите тренировочные данные (**Training data**). Их можно загрузить через Microsoft Foundry или из локального окружения.
...
12. После завершения тонкой настройки статус модели изменится на **Completed**, как показано на изображении ниже. Теперь вы можете развернуть модель и использовать её в собственном приложении, в playground или в prompt flow. Подробнее см. [Как развернуть семейство малых языковых моделей Phi-3 с помощью Microsoft Foundry](https://learn.microsoft.com/azure/ai-studio/how-to/deploy-models-phi-3?tabs=phi-3-5&pivots=programming-language-python).

    ![FineTuneSelect](../../../../translated_images/ru/completed.4dc8d2357144cdef.webp)

> [!NOTE]
> Для более подробной информации о тонкой настройке Phi-3 посетите страницу [Fine-tune Phi-3 models in Microsoft Foundry](https://learn.microsoft.com/azure/ai-studio/how-to/fine-tune-phi-3?tabs=phi-3-mini).

## Удаление тонко настроенных моделей

Вы можете удалить тонко настроенную модель из списка моделей для тонкой настройки в [Microsoft Foundry](https://ai.azure.com) или со страницы с деталями модели. Выберите модель для удаления на странице тонкой настройки и нажмите кнопку Delete.
...
Модели Phi, тонко настроенные как сервис, предоставляются Microsoft и интегрированы с Microsoft Foundry для использования. Цены можно узнать при [развертывании](https://learn.microsoft.com/azure/ai-studio/how-to/deploy-models-phi-3?tabs=phi-3-5&pivots=programming-language-python) или тонкой настройке моделей во вкладке Pricing and terms в мастере развертывания.
...
- Разверните модель как веб-сервис в Microsoft Foundry.
